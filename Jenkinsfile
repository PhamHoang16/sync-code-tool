pipeline {
    ...
    parameters {
        ...
        string(name: 'DEST_USER', defaultValue: '', description: 'Destination Username (Optional for some PATs)')
        password(name: 'DEST_TOKEN', defaultValue: '', description: 'Destination Access Token or Password')
    ...
    stages {
        ...
        stage('Execute Sync') {
            steps {
                script {
                    // Securely assign manual credentials to env
